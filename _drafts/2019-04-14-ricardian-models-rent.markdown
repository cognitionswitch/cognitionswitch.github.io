...
In _Principles of Political Economy_, Ricardo formulates a model to understand how the price of rent relates to the price of goods produced on leased land. Specifically, his model concerns the rent charged for lands used for production or commercial enterprise. The model explains the circumstances under which land owners may charge rent, the variation in the price of rent, and the influences of population and agricultural innovation on the price of rent.
...
Ricardo states that rent is only charged where land utilization achieves an equal or higher return on capital than the common rate of profits. A capitalist would forego the opportunity to utilize land if the deduction of rent left a rate of return less profitable than returns realized in other ventures; in these cases, capitalists would abandon the venture requiring land utilization and redeploy their capital in an enterprise where common rates of profit are readily achievable. 

In contrast, wherever land utilization achieves a return higher than the common rate of profit affords, land owners can charge rent on the return in excess of the common rate of profit. If a capitalist refused to pay this rent, other entrepreneurs would come forward who would be satisfied by the common rate of profit and who would pay any excess to the land owner. When land of inferior quality is taken for production ventures, it is because utilization of this land earns at least the common rate of profit. And since entrepreneurs can still achieve the common rate of profit using less productive land, owners of more productive land can demand rent.
...
That is, rent is charged where profitability in land utilization exceeds the common rate of profits.

_Variation in rent_.<br>
Under Ricardo's framework, rent varies according to the value of commodities produced with land utilization for a fixed capital investment. Lands well possessed of qualities useful for production will charge more rent than those lands lacking these properties. Let $$v_{i,j}$$ represent the valuation of commodities produced from an investment $$c_{i,j}$$. The rent on the $$j^{th}$$ lot of land is
...
- less productive land is taken for rent when aggregate demand raises prices high enough to justify its utilization.
...
- according to Ricardo, valuation is determined by the [_quantity_ of labor](../../../2019/04/07/ricardo-value-in-exchange.html) and capital requisit for production.
...
With an increase to population, the demand for land utilization will increase valuation of goods $$v_{h,k}$$ produced with an $$h^{th}$$ capital expenditure on the $$k^{th}$$ lot of land: $$v_{k,h} \propto P(t)$$. Under a Ricardian framework, the function $$P(t)$$ is related to wages and is discussed in the context of the wage-fund model. Here, it is only important to note that population increase leads to increase in $$v_{h,k}$$, which in turn increases returns on capital $$\frac{v_{h,k} - c_{h,k}}{c_{h,k}}$$ until they equal or exceed $r_0$. When profitability for land utilization reaches this level, $c_{h,k}$ will be deployed so that $$v_{h,k} \in V$$. Thus, by equation $\ref{c}$,
...
Improvements to land can also influence the rent charged on the land. By improvements, Ricardo means those developments that abridge labor for production of commodities. Two types of improvements are explored: 
1. Those improvements which enhance the productivity of the land.
2. Those improvements that abridge labor through employment of machinery.

For improvements that enhance productivity of land, Ricardo imagines a development that simply adds a constant surplus $k$ to every capital investment advanced toward land utilization. Prior to enhancements, aggregate valuation of commodities utilizing land amounts to $v_U$, computed in equation $\ref{c}$. Since the demand for land utilization remains unchanged, a constant surplus $k$ added to all production utilizing land would over-supply effective demand. This over-supply will lead to withdrawal of some capital investments toward land utilization that would otherwise yield a return lower than $r_0$.
...
$$
\begin{eqnarray}
V' &=& \left\lbrace v_{i,j}' \middle\vert \frac{v_{i,j}' - c_{i,j}}{c_{i,j}} > r_0 \land \sum v_{i,j}' = v_U \right\rbrace 
\end{eqnarray}
\label{f}\tag{6}
$$

Since $$\sum\limits_{v' \in V'} v' = \sum\limits_{v \in V} v$$ (equations $\ref{c}$ and $\ref{e}$), some capital investements $c_{i,j}$ on lands that are least productive will be withdrawn since any oversupply of commodities will yield a return lower than the common rate of profits. Thus, enhancements to productivity of this sort will tend to lower aggregate rent $$R_U$$. This reduction accords with the fact that the addition of $$k$$ to all valuations of commodities produced through land utilization effectively compresses the variation in productivity of land.
...
Let $$w_{i,j} \leq 1$$ represent a reduction of the $$i^{th}$$ expenditure on the $$j^{th}$$ plot of land to produce commodities with valuation $$v_{i,j}$$. The return on capital is $$r_{i,j} = \frac{v_{i,j} - w_{i,j}c_{i,j}}{w_{i,j}c_{i,j}}$$. Since the introduction of machinery does not enhance productivity of the current land in use, land utilization does not decrease. But since employment of machinery decreases the labor necessary to produce the commodities achieved through land utilization, machinery may still influence the price of rent. When weights $w_{i,j}$ are very small for capital investments that just clear the common rate of profit relative to those investments toward the most productive land, rent will decrease with the decrease in variation of land productivity. If instead $w_{i,j}$ are much smaller for capital invested toward the most productive land, then variation in land productivity will increase and rents will rise on those lands along with it.	 
